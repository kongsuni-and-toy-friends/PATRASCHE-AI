...
class detector:
    def __init__(self):
        ...
        with open(os.environ['CHATBOT_ROOT']+"/resources/bad_words/bad_words.txt", encoding='UTF-8') as file:
            bad_words = file.read()
            self.bad_words_list = bad_words.split(',')

        with open(os.environ['CHATBOT_ROOT']+'/resources/bad_words/bad_embedding_SBERT.pickle','rb') as f:
            self.embeddings_SBERT = pickle.load(f)

        with open(os.environ['CHATBOT_ROOT']+'/resources/bad_words/bad_embedding_BERT.pickle','rb') as f:
